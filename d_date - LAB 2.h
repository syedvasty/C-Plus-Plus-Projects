...
bool date::operator>(date rhs)																// member function to check if one date is greater than another
{
	if (year > rhs.year)
		return true;
...
bool date::operator<(date rhs)																// member function to check if one date is less than another
{
	if (year < rhs.year)
		return true;
	else if (year == rhs.year && month < rhs.month)
...
void date::operator++()																		// function to increment the date by one day
{
	incrementDate(1);
}

ostream& operator<<(ostream& ostr, date rhs)												// function to output the date in cout
{
	ostr << rhs.month << "/" << rhs.day << "/" << rhs.year;
	return ostr;
}

istream& operator>>(istream& istr, date& rhs)												// function to take in a date in cin
{
	char temp;
	int d, m, y;
